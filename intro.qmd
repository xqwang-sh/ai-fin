...
#### **命令行工具：Claude Code & Codex CLI**

对于喜欢在终端工作的开发者，命令行式的 AI 编程助手提供了更轻量、更灵活的选择：

1. **Claude Code** (Anthropic 官方)

Claude Code 是 Anthropic 官方推出的命令行 AI 编程助手，提供与 Claude 的原生集成：
...
2. **Codex CLI** (基于 ChatGPT)

Codex CLI 是基于 OpenAI 模型的命令行编程助手，可以在终端中调用 GPT-4 进行代码生成：

- **优势**：
  - 轻量级，启动快速
  - 支持 GPT-4 及其变体（GPT-4o、o1 等）
  - 可配置不同的 AI 模型
  - 开源社区活跃，插件生态丰富
...
- **优势**：更轻量、启动快、适合服务器环境、易于脚本化
- **劣势**：缺少可视化界面、需要手动管理上下文、调试体验不如 IDE
- **建议**：将命令行工具作为 Cursor 的补充，在快速任务和远程开发时使用
:::
...
- **Cursor**：功能最全面，Agent 模式最强大，推荐作为主力 IDE 工具
- **Claude Code / Codex CLI**：轻量级命令行工具，适合：
  - Claude Pro 或 ChatGPT Plus 用户
  - 远程服务器开发
  - 快速原型开发和脚本任务
  - 作为 IDE 的补充工具
  - 支持 GLM 4.7
...
**推荐组合：**
- 主力开发：Cursor IDE（图形界面 + Agent 能力）
- 辅助工具：Claude Code 或 Codex CLI（快速任务、远程开发）
- 备选方案：国内工具（网络问题时）

建议先从 Cursor 或国内工具（如 Kimi Code）开始，掌握基本工作流后，再根据具体需求添加命令行工具作为补充。
:::
...
2. [ ] （可选）安装命令行辅助工具：
   - Claude Code（适合 Claude Pro 用户）
   - Codex CLI（适合 ChatGPT Plus 用户）

3. [ ] （可选）安装至少一个 MCP Server（推荐 Context7，Cursor 和 Claude Code 支持）

4. [ ] （可选）安装 Superpowers Skills（Cursor 支持）

**动手实践：**

5. [ ] 尝试使用 AI 工具完成一个简单任务（如创建 Python 项目模板）
6. [ ] （Cursor 用户）尝试使用 Plan 模式（`Shift+Tab`）规划一个小项目
7. [ ] 测试 AI 工具的代码生成和解释能力

**理论学习：**

8. [ ] 阅读第1周必读材料：Hastie et al. (2009) Chapters 3, 7
9. [ ] 思考：你的研究方向如何与 AI 方法结合？
:::
